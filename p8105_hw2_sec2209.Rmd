...
In the following code chunk, I read in the monthly poll data, transform the date variable into two variables for year and month name, and create a _president_ variable taking the political party values of _gop_ and _dem_. 
...
In the next code chunk I read and clean in the SNP data following the same steps as above.
...
Finally, I read and clean the unemployment data. These data are not tidy, so I transpose the data from the wide to long format. Once these data are tidy, I merge the polls, SNP, and unemployment data.
...
The pols-month data contain information on the number of national politicians who are democratic or republican 
The resulting data set contains `r nrow(pols_snp_unem)` observations and `r ncol(pols_snp_unem)` variables. The data range from `r pull(pols_snp_unem, year) %>% min()` to `r pull(pols_snp_unem, year) %>% max()`.